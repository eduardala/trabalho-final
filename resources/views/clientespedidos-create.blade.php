...
<head>
    ...
    <title>Cadastrar um novo cliente - pedido</title>
</head>
<body>
    ...
    <div id="main" class="container-fluid">
        <h3 class="page-header">Cadastrar Cliente/Pedido</h3>
        <form action="{{ route('cadastro-pc') }}" method="POST">
            @csrf
            <div class="row">
                <div class="form-group col-md-4">
                    ...
                    <option value="{{ $cliente->id }}"
                        @if(isset($pedidosclientes->clientes_id) && ($pedidosclientes->clientes_id == $cliente->id))
                            {{ 'selected' }}
                        @endif
                            >
                        {{ $cliente->nome }}</option>
                ...
                <div class="form-group col-md-12">
                    <label>Escolha o Pedido (Descrição - Valor)</label>
                    <div class="col-md-3 form-group">
                    <select name="pedidos_id" id="pedidos_id" class="form-control">
                        @if (isset($pedidos))
                        @foreach($pedidos as $pedido)
                        <option value="{{ $pedido->id }}"
                            @if(isset($pedidosclientes->pedidos_id) && ($venda->pedidos_id == $pedido->id))
                                {{ 'selected' }}
                            @endif
                                >
                            {{ $pedido->descricao.' - '.$pedido->valor }}</option>
                        @endforeach
                        @endif
                    </select>
                </div><br>
                <div id="actions" class="row">
                    <div class="col-md-12">
                      ...
                      <a href="/pc" class="btn btn-default">Cancelar</a>
                </div>
            </div>
        </form>
    </div>
